...
import { KegiatanEditMode as ZKegiatan } from "@/zod/schemas/kegiatan";
...
export const setupKegiatanEdit = async (
  kegiatan: ZKegiatan
  ...
  try {
    ...
    if (!fs.existsSync(filePesertaXlsxPathResolvedPath)) {
      console.error("W-KEG-02 File peserta tidak ditemukan, skipping");
      // return {
      //   success: false,
      //   error: "E-KEG-02",
  ...
  try {
    const kegiatanBaru = await dbHonorarium.$transaction(async (prisma) => {
      ...
      const kegiatanBaru = await updateKegiatan(
        prisma,
        kegiatan,
        satkerId,
        unitKerjaId,
    ...
    if (logUploadedFile.length === 0) {
      console.error("W-FSK01 No file found in temp folder, skipping");
      // return {
      //   success: false,
...
async function recreateDetailUH(
  prisma: Prisma.TransactionClient,
  kegiatan: ZKegiatan,
  penggunaId: string
) {
  ...
  // select all peserta kegiatan
  const pesertaKegiatan = await prisma.pesertaKegiatan.findMany({
    where: {
      kegiatanId: kegiatanBaruId,
    },
  });

  // delete all detail UH
  await prisma.uhDalamNegeri.deleteMany({
    where: {
      pesertaKegiatanId: {
        in: pesertaKegiatan.map((peserta) => peserta.id),
      },
    },
  });
  await prisma.uhLuarNegeri.deleteMany({
    where: {
      pesertaKegiatanId: {
        in: pesertaKegiatan.map((peserta) => peserta.id),
      },
      ...
      for (const it of itinerary) {
        ...
        const uangHarian = await prisma.uhLuarNegeri.create({
          data: {
            ...
            createdBy: penggunaId,
            golonganUh: peserta.golonganUhLuarNegeri,
          },
        });
      }
    }
...
async function insertPesertaDariExcel(
  ...
  try {
    if (pesertaKegiatan.length > 0) {
      //hapus dulu peserta kegiatan lama jika ada yang baru
      const deletePesertaKegiatan = await prisma.pesertaKegiatan.deleteMany({
        where: {
          kegiatanId: kegiatanBaruId,
        },
      });
    } else {
      // recreate detailnya saja
      return await recreateDetailUH(prisma, kegiatan, penggunaId);
    }
...
const insertItinerary = async (
  ...
  // jika ada itinerary baru maka yg lama dihapus
  const deletedItinerary = await prisma.itinerary.deleteMany({
    where: {
      kegiatanId: kegiatanBaruId,
    },
  });
...
export default setupKegiatanEdit;
